        ...
        <header class="fm-heading">
            ...
            <div class="fm-heading__actions">
                @if ($canManageProjectFiles)
                    <button class="button" type="button" data-upload-open><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 16V4"/><path d="m7 9 5-5 5 5"/><path d="M5 14v5h14v-5"/></svg>Add files</button>
                    ...
                    <div class="fm-file-list"><div class="fm-file-row fm-file-row--head"><span>Name</span><span>Location</span><span>Owner</span><span>Updated</span><span>Access</span><span>Actions</span></div>
                        @foreach ($files as $file)
                            <article class="fm-file-row" data-project-file-item="{{ $file->id }}">
                                ...
                                <div class="fm-file-cell"><small>Access</small><span class="file-access-badge file-access-badge--{{ $file->is_shared ? 'shared' : 'private' }}">@if($file->is_shared)<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9.5 14.5 14.5 9.5"/><path d="m7.5 16.5-1.7 1.7a3 3 0 0 1-4.2-4.2l3.2-3.2A3 3 0 0 1 9 10.7"/><path d="M15 13.3a3 3 0 0 1 .2-4.1L18.4 6a3 3 0 0 1 4.2 4.2l-1.7 1.7"/></svg>Shared @else<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="5" y="10" width="14" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg>Private @endif</span></div>
            ...
            <div id="upload-document" class="fm-upload-modal" data-upload-modal hidden>
                ...
                <section class="fm-upload-dialog" role="dialog" aria-modal="true" aria-labelledby="upload-dialog-title">
                    <header class="fm-upload-dialog__head">
                        <div><span class="eyebrow">Add to the library</span><h2 id="upload-dialog-title">Upload files</h2><p>Files remain private until you create a share link.</p></div>
                    ...
                    <form method="POST" action="{{ route('admin.projects.files.external.store') }}" enctype="multipart/form-data" data-project-file-upload>
                        ...
                        <div class="field fm-file-picker">
                            <label for="external-project-file">Documents</label>
                            <div class="fm-dropzone" data-file-dropzone role="button" tabindex="0" aria-describedby="file-upload-help">
                                <input id="external-project-file" type="file" name="files[]" multiple aria-required="true" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.ppt,.pptx,.txt,.rtf,.jpg,.jpeg,.png,.webp,.zip" data-file-input>
                                <span class="fm-dropzone__icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M12 16V5"/><path d="m7.5 9.5 4.5-4.5 4.5 4.5"/><path d="M5 15v4h14v-4"/></svg></span>
                                <span class="fm-dropzone__copy"><strong>Drop files here</strong><small>or click to browse</small></span>
                                <span class="fm-dropzone__limit">Up to 10 files</span>
                            </div>
                            <small id="file-upload-help">PDF, Office, image, text or ZIP · maximum 50 MB per file</small>
                            <div class="fm-upload-queue" data-file-queue hidden aria-live="polite"></div>
                        </div>
                        ...
                        @error('document_scope')<p class="form-error">{{ $message }}</p>@enderror @error('project_id')<p class="form-error">{{ $message }}</p>@enderror @error('folder')<p class="form-error">{{ $message }}</p>@enderror @error('files')<p class="form-error">{{ $message }}</p>@enderror @error('files.*')<p class="form-error">{{ $message }}</p>@enderror @error('file')<p class="form-error">{{ $message }}</p>@enderror
                        ...
                        <footer class="fm-upload-dialog__actions"><button class="ghost-button" type="button" data-upload-close>Cancel</button><button class="button" type="submit" data-project-file-submit>Upload files</button></footer>
                    </form>
                </section>
            </div>
        @endif
...
<style>
    ...
    .fm-file-picker{padding:0;border:0;background:transparent}.fm-dropzone{position:relative;display:flex;min-height:104px;align-items:center;gap:13px;padding:16px;border:1.5px dashed #ccb66f;border-radius:12px;background:#fffcf5;cursor:pointer;transition:.16s ease}.fm-dropzone:hover,.fm-dropzone:focus-visible,.fm-dropzone.is-dragging{border-color:#a87908;background:#fff7e5;box-shadow:0 0 0 3px rgba(184,134,11,.08);outline:0}.fm-dropzone input[type=file]{position:absolute;width:1px;height:1px;min-height:0;padding:0;overflow:hidden;opacity:0;pointer-events:none}.fm-dropzone__icon{display:grid;flex:0 0 auto;width:42px;height:42px;place-items:center;border-radius:11px;background:#f2c34f;color:#6f4c00}.fm-dropzone__icon svg{width:20px;height:20px}.fm-dropzone__copy{display:block;min-width:0}.fm-dropzone__copy strong,.fm-dropzone__copy small{display:block}.fm-dropzone__copy strong{font-size:13px}.fm-dropzone__copy small{margin-top:3px;color:#877e70;font-size:10px}.fm-dropzone__limit{margin-left:auto;padding:4px 8px;border-radius:99px;background:#f1ede4;color:#81786b;font-size:9px;font-weight:800;white-space:nowrap}.fm-file-picker>small{display:block;margin-top:7px;color:#8a8175;font-size:10px}.fm-upload-queue{display:grid;max-height:168px;gap:6px;margin-top:10px;overflow-y:auto}.fm-upload-queue[hidden]{display:none}.fm-upload-queue__item{display:flex;align-items:center;gap:9px;min-height:42px;padding:7px 9px;border:1px solid #e7e2d9;border-radius:9px;background:#faf9f6}.fm-upload-queue__type{display:grid;flex:0 0 auto;width:28px;height:28px;place-items:center;border-radius:7px;background:#fff0cc;color:#8d6405;font-size:8px;font-weight:900}.fm-upload-queue__name{min-width:0;flex:1}.fm-upload-queue__name strong,.fm-upload-queue__name small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.fm-upload-queue__name strong{font-size:10px}.fm-upload-queue__name small{margin-top:2px;color:#8b8378;font-size:8px}.fm-upload-queue__remove{display:grid;flex:0 0 auto;width:28px;height:28px;place-items:center;padding:0;border:0;border-radius:7px;background:transparent;color:#9a6258;cursor:pointer}.fm-upload-queue__remove:hover{background:#fff0ed;color:#b33f31}.fm-upload-queue__remove svg{width:13px;height:13px}
    .fm-file-cell>.file-access-badge{display:inline-flex;width:max-content;min-height:24px;margin-top:0;overflow:visible;color:var(--file-access-color);font-size:10px;font-weight:700;letter-spacing:0;text-overflow:clip}.fm-file-cell>.file-access-badge svg{display:block;width:12px;height:12px}
...
<script>
    (() => {
        ...
        const fileQueue = form.querySelector('[data-file-queue]');
        const maxFiles = 10;
        let selectedFiles = [];
        ...
        const defaultLabel = submit?.textContent || 'Upload files';
        const fileKey = (file) => `${file.name}:${file.size}:${file.lastModified}`;
        const formatFileSize = (bytes) => { if (bytes < 1024) return `${bytes} B`; const units = ['KB', 'MB', 'GB']; let value = bytes / 1024; let unit = units[0]; for (let index = 1; value >= 1024 && index < units.length; index += 1) { value /= 1024; unit = units[index]; } return `${value >= 10 ? value.toFixed(0) : value.toFixed(1)} ${unit}`; };
        const renderFileQueue = () => {
            if (!fileQueue) return;
            fileQueue.replaceChildren();
            fileQueue.hidden = selectedFiles.length === 0;
            selectedFiles.forEach((file, index) => {
                const item = document.createElement('div'); item.className = 'fm-upload-queue__item';
                const type = document.createElement('span'); type.className = 'fm-upload-queue__type'; type.textContent = (file.name.split('.').pop() || 'FILE').slice(0, 4).toUpperCase();
                const details = document.createElement('span'); details.className = 'fm-upload-queue__name';
                const name = document.createElement('strong'); name.textContent = file.name;
                const size = document.createElement('small'); size.textContent = formatFileSize(file.size);
                const remove = document.createElement('button'); remove.type = 'button'; remove.className = 'fm-upload-queue__remove'; remove.setAttribute('aria-label', `Remove ${file.name}`); remove.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg>';
                remove.addEventListener('click', () => { selectedFiles.splice(index, 1); renderFileQueue(); });
                details.append(name, size); item.append(type, details, remove); fileQueue.append(item);
            });
        };
        const addFiles = (files) => {
            const knownFiles = new Set(selectedFiles.map(fileKey));
            const candidates = Array.from(files).filter((file) => { const key = fileKey(file); if (knownFiles.has(key)) return false; knownFiles.add(key); return true; });
            const availableSlots = Math.max(0, maxFiles - selectedFiles.length);
            selectedFiles.push(...candidates.slice(0, availableSlots));
            renderFileQueue();
            if (status) { const exceedsLimit = candidates.length > availableSlots; status.classList.toggle('form-error', exceedsLimit); status.textContent = exceedsLimit ? `Only ${maxFiles} files can be uploaded at once.` : ''; }
        };
        ...
        const upload = (formData, fileCount) => new Promise((resolve, reject) => { const request = new XMLHttpRequest(); const noun = fileCount === 1 ? 'document' : 'documents'; request.open('POST', form.action); request.setRequestHeader('Accept', 'application/json'); request.setRequestHeader('X-Requested-With', 'XMLHttpRequest'); request.upload.addEventListener('progress', (event) => setProgress(event.lengthComputable ? (event.loaded / event.total) * 100 : 0, `Uploading ${fileCount} ${noun}`, event.lengthComputable ? `${Math.round((event.loaded / event.total) * 100)}% uploaded` : 'Uploading securely…')); request.addEventListener('load', () => { let payload = {}; try { payload = JSON.parse(request.responseText || '{}'); } catch (error) {} if (request.status >= 200 && request.status < 300) return resolve(payload); reject(new Error(Object.values(payload.errors || {}).flat()[0] || payload.message || 'The documents could not be uploaded.')); }); request.addEventListener('error', () => reject(new Error('The documents could not be uploaded.'))); request.send(formData); });
        form.addEventListener('submit', async (event) => { event.preventDefault(); if (!submit) return; if (selectedFiles.length === 0) { if (status) { status.textContent = 'Choose at least one document to upload.'; status.classList.add('form-error'); } dropzone?.focus(); return; } const fileCount = selectedFiles.length; const formData = new FormData(form); formData.delete('files[]'); selectedFiles.forEach((file) => formData.append('files[]', file, file.name)); submit.disabled = true; submit.textContent = `Uploading ${fileCount}…`; progress?.classList.remove('is-complete', 'is-error'); setProgress(0, 'Preparing upload', fileCount === 1 ? 'Getting the document ready…' : `Getting ${fileCount} documents ready…`); if (status) { status.textContent = ''; status.classList.remove('form-error'); } try { const payload = await upload(formData, fileCount); progress?.classList.add('is-complete'); setProgress(100, 'Upload complete', fileCount === 1 ? 'Document is now available in the library.' : `${fileCount} documents are now available in the library.`); if (status) status.textContent = payload.message || `${fileCount} document${fileCount === 1 ? '' : 's'} uploaded successfully.`; window.setTimeout(() => window.location.reload(), 700); } catch (error) { progress?.classList.add('is-error'); if (progressLabel) progressLabel.textContent = 'Upload failed'; if (progressDetail) progressDetail.textContent = 'Check the documents and try again.'; if (status) { status.textContent = error.message; status.classList.add('form-error'); } } finally { submit.disabled = false; submit.textContent = defaultLabel; } });
    })();
</script>
@endpush
