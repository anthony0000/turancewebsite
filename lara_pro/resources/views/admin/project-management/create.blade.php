...
@section('project-content')
    <section class="panel pm-panel"><div class="pm-panel-head"><div><span class="eyebrow">Foundation</span><h3>Create a project workspace</h3><p>Set the delivery context once; the board, task keys, and team space will be created with it.</p></div></div>
        <form method="POST" action="{{ route('admin.project-management.projects.store') }}">@csrf
            <div class="pm-form-grid">
                ...
                <div class="field-full"><label>Team members</label><div class="pm-form-grid" style="grid-template-columns:repeat(3,minmax(0,1fr)); margin-top:8px">@foreach ($members as $member)<label class="pm-chip" style="justify-content:flex-start; gap:7px"><input type="checkbox" name="member_ids[]" value="{{ $member->id }}" @checked(in_array($member->id, old('member_ids', [])))> {{ $member->name }}</label>@endforeach</div></div>
