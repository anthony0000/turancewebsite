...
it('presents a search focused homepage with direct conversion paths', function () {
    $this->get('/')
        ...
        ->assertSee('tt-contact-cta__notice', false)
        ->assertSee('Proposal materials are confidential.')
        ->assertSee(route('terms.show', absolute: false).'#proposal-design-rights', false)
...
it('renders linked privacy and terms pages with current public metadata', function () {
    ...
    $this->get('/terms')
        ...
        ->assertSee('tt-legal-page', false)
        ->assertSee('tt-legal-hero__summary', false)
        ->assertSee('tt-legal-warning', false)
        ->assertSee('Important use restriction')
        ->assertSee('Proposal materials are confidential.')
        ->assertSee('href="#proposal-design-rights"', false)
        ->assertSee('Proposals, concepts and design rights')
        ->assertDontSee('tt-legal-section--protected', false)
        ->assertSee('receiving or reviewing those materials grants no right to copy')
        ->assertSee('The Copyright Act, 2022 protects qualifying original expression rather than abstract ideas alone')
