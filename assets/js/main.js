...
(function ($) {
    ...
    var windowOn = $(window);
    var scrollSmoother = null;
    var prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    function scrollToPosition(position, shouldAnimate = true) {
        var top = Math.max(0, Number(position) || 0);
        var animate = shouldAnimate && !prefersReducedMotion;

        if (scrollSmoother) {
            scrollSmoother.scrollTo(top, animate);
            return;
        }
    ...
    function smoothScroll() {
        $('.smooth a').on('click', function (event) {
            ...
            if (target.length) {
                ...
                scrollToPosition(target.offset().top - 120);
            }
        });
    }
    smoothScroll();
    if ($('#smooth-wrapper').length && $('#smooth-content').length) {
        ...
        var canUseScrollSmoothing = window.matchMedia('(min-width: 1024px) and (pointer: fine)').matches
            && !prefersReducedMotion;

        if (canUseScrollSmoothing) {
    ...
    function scrollNav() {
        $('.wt-onepage-menu li a').click(function () {
            ...
            scrollToPosition($($(this).attr('href')).offset().top - 80);
            return false;
        });
    }
    scrollNav();
...
document.addEventListener("DOMContentLoaded", function () {
    var testimonialImages = document.querySelectorAll(
        ".wt-testimonial-img img, .wt-testimonial-2-img img, .wt-testimonial-3-img img, .wt-testimonial-4-wrapper img"
    );
    ...
    testimonialImages.forEach(function (image) {
        function applyFallback() {
            ...
            if (!wrapper || wrapper.classList.contains("wt-image-fallback")) {
                return;
            }

            var altText = (image.getAttribute("alt") || "").trim();
            var initials = altText
                .split(/\s+/)
                .filter(Boolean)
                .slice(0, 2)
                .map(function (chunk) {
            ...
            wrapper.classList.add("wt-image-fallback");
            wrapper.setAttribute("data-placeholder", initials || "TT");
